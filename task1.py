...
# A (3,6); B (2,1) -> 5,09
print('2. Расстояние между двумя точками в 2D пространстве (ввод координат в коде): ')
def AB(xy1,xy2):
    length_AB = math.sqrt(pow((xy2[0] - xy1[0]), 2) + pow((xy2[1] - xy1[1]), 2))
    print(f'Длина отрезка AB между точками A({xy1[0]},{xy1[1]}) и B({xy2[0]},{xy2[1]}) равна {round(length_AB,3)}')
AB([3,6],[2,1])
...
# 1 -> x > 0, y > 0
print('3. По заданной четверти показывает диапазон возможных координат: ')
def quarter(number_name):
    match number_name:
        case 1: print(f'{number_name} - x > 0, y > 0')
        case 2: print(f'{number_name} -  x < 0, y > 0')
        case 3: print(f'{number_name} - x < 0, y < 0')
        case 4: print(f'{number_name} - x > 0, y < 0')
        case _: print('Нужно ввести число от 1 до 4, попробуйте снова :)')

quater_number = int(input('Введите номер четверти [1, 4]: '))
quarter(quater_number)
